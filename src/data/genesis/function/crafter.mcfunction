...
function ~/tick:
    ...
    as @e[type=item_display,tag=genesis.crafter] at @s function ~/../tick_crafter:
        unless block ~ ~-1 ~ barrel positioned ~ ~-.5 ~ return run function ~/../break_crafter:
            ...
            loot replace entity @n[type=minecraft:item,distance=..3,nbt={Age:0s,Item:{id:"minecraft:barrel"}}] contents loot genesis:bolt-item/crafter
            kill @s
...
class Crafter:
    ...
    custom_data = {genesis:{placeable:{command:"function genesis:crafter/place"}}}

def table(item):
    loot_table f'{item.namespace}:bolt-item/{item.id}' {"pools": [{"rolls": 1,"entries": [{"type": "minecraft:item","name": f'minecraft:{item.base_item}',"functions": [{"function": "minecraft:set_components","components": item.components}]}]}]}
table(Crafter)
...
def custom_recipe(recipe: list[list], output):
    command = "execute "
    slot = 2
    for line in recipe:
        for item in line:
